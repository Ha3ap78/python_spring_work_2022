...
def letter_1(gu, bo, ans):
    bo[ans.index(gu)] = gu
    print("Здорово! Есть такая буква! Откройте букву", gu, "на табло!")
    print(bo)


def letter_2(gu, bo, ans):
    indices = [i for i, x in enumerate(ans) if x == gu]
    bo[indices[0]] = gu
    bo[indices[1]] = gu
    print("Здорово! Есть такая буква! Откройте букву", gu, "на табло!")
    print(bo)
        ...
        for i in answer:
            if guess in answer:
                if answer.count(guess) == 1:
                    letter_1(guess, board, answer)
                    break
                if answer.count(guess) > 1:
                    letter_2(guess, board, answer)
                    break
